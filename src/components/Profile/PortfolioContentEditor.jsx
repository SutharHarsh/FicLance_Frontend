...
export default function PortfolioContentEditor({ user, onUpdate }) {
  ...
  useEffect(() => {
    if (user) {
      ...
      console.log(
        ...
        user.profile?.portfolio?.experiences
      );
      console.log(
        "📋 Portfolio manual projects:",
        user.profile?.portfolio?.manualProjects
      );
      console.log(
        "📋 Custom sections:",
        user.profile?.portfolio?.customSections
      );
  ...
  const removeSkill = (skillToRemove) => {
    setContent({
      ...content,
      skills: content.skills.filter((s) => s !== skillToRemove),
    });
  };
  ...
  const removeCustomLink = (index) => {
    ...
    setContent({
      ...content,
      contact: { ...content.contact, customLinks: newLinks },
    });
  };
            ...
            {content.experiences.length === 0 ? (
              ...
              <div className="space-y-4">
                {content.experiences.map((exp, idx) => (
                  <div
                    key={idx}
                    className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-2xl p-6 space-y-4"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {exp.startDate && (
                        <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                          <p className="text-sm text-blue-800 dark:text-blue-300 font-medium">
                            Preview: {formatDate(exp.startDate)} -{" "}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                  Add personal projects. Platform projects will also appear
                  automatically.
                </p>
              </div>
              <button
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <MonthPicker
                            ...
                            onChange={(value) =>
                              updateProject(idx, "startDate", value)
                            }
                        ...
                        <div>
                          ...
                          <MonthPicker
                            ...
                            onChange={(value) =>
                              updateProject(idx, "endDate", value)
                            }
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (val === "" || /^[0-9+\-\s()]*$/.test(val)) {
                      setContent({
                        ...content,
                        contact: { ...content.contact, phone: val },
                      });
                    }
                  }}
            ...
            {content.customSections.length === 0 ? (
              ...
              <div className="space-y-4">
                {content.customSections.map((section, idx) => (
                  <div
                    key={idx}
                    className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-2xl p-6 space-y-4 shadow-sm"
                  >
                    <div className="flex items-center justify-between">
